export function createFoligoClient(config) {
  ...
  const getBlogs = async () => {
    ...
    return filtered.map((c) => {
      ...
      const id = normalizeId(c)
      const itemWithLinks = attachContentLinks(c, siteData)
      return { 
        id, // Keep ID for backwards compatibility
        slug: slug || id || c.uid, // Use slug, fallback to ID if no slug
        ...itemWithLinks,
        // Ensure tags and linkedSkills are preserved
  ...
  const getBlogBySlug = async (slug) => {
    ...
    // Attach contentLinks and ensure tags/linkedSkills are present
    const itemWithLinks = attachContentLinks(found, siteData)
    return {
      ...itemWithLinks,
      tags: itemWithLinks.tags || [],
      linkedSkills: itemWithLinks.linkedSkills || [],
