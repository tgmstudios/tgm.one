export function createFoligoClient(config) {
  ...
  const stripInvalidMarkdownLinks = (value) => {
    if (typeof value !== 'string') return value
    if (!value.trim()) return value
    return value.replace(/(!?)\[(.+?)\]\(\s*(null|undefined|nan)\s*\)/gi, (_, isImage, text) => {
      return isImage ? '' : text
    })
  }

  const stripInvalidHtmlLinks = (value) => {
    if (typeof value !== 'string') return value
    if (!value.trim()) return value
    return value
      .replace(/<a\b([^>]*)href=["']\s*(null|undefined|nan)\s*["']([^>]*)>(.*?)<\/a>/gi, '$4')
      .replace(/<img\b([^>]*)src=["']\s*(null|undefined|nan)\s*["'][^>]*>/gi, '')
  }

  const sanitizeContentFields = (item = {}) => {
    if (!item || typeof item !== 'object') return item
    return {
      ...item,
      content: stripInvalidMarkdownLinks(item.content),
      body: stripInvalidMarkdownLinks(item.body),
      markdown: stripInvalidMarkdownLinks(item.markdown),
      html: stripInvalidHtmlLinks(item.html)
    }
  }
  ...
  const ensureProjectShape = (project, siteData) => {
    ...
    const safeContent = sanitizeContentFields(itemWithLinks)
  ...
  const getBlogs = async () => {
    ...
    // Get blogs directly from content.blogs array to preserve original order (0, 1, 2, etc.)
    const blogsArray = siteData.content.blogs || []
    
    const filtered = blogsArray.filter((c) => {
    ...
    // Preserve original array order (0, 1, 2, etc.)
    return filtered
  ...
  const getExperiences = async () => {
    ...
    // Get experiences directly from content.experiences array to preserve original order (0, 1, 2, etc.)
    const experiencesArray = siteData.content.experiences || []
    
    // Preserve original index before filtering
    const experiencesWithIndex = experiencesArray.map((c, index) => ({
      ...c,
      _originalIndex: index
    }))
    
    const filtered = experiencesWithIndex.filter((c) => {
      // Filter out revisions
      if (isRevision(c)) return false
      // Filter by contentType and status
      const contentType = (c.contentType || '').toString().toUpperCase()
    ...
    // Preserve original array order (0, 1, 2, etc.) - sort by _originalIndex
    return filtered
      .map((c) => ({
        ...c,
        id: normalizeId(c),
        _originalIndex: c._originalIndex // Explicitly preserve original index
      }))
      .sort((a, b) => {
        const indexA = a._originalIndex ?? Infinity
        const indexB = b._originalIndex ?? Infinity
